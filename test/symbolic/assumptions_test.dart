...
import 'package:latex_math_evaluator/src/symbolic/symbolic_engine.dart';
...
void main() {
  group('Assumptions System', () {
    ...
    test('Engine integration with PowerPowerRule', () {
      ...
      // My simple rule requires x >= 0 to simplify at all.
      ...
      // Should become x^(2*0.5) = x^1 = x
      // 2*0.5 = 1.0.
      // Result: BinaryOp(x, power, 1.0).
      // Then PowerOneRule (x^1 = x) should apply if 1.0 is treated as 1.
      // Wait, 1.0 is a double. PowerOneRule checks for 1.
      // NumberLiteral value is double. 1.0 == 1 is true in Dart.

      // However (x^2)^0.5 -> x^(1.0).
      // Then PowerOneRule matches value == 1.

      final expected = x;
      // Wait, 2*0.5 might be 1.0.
